...
[npx skills](https://github.com/vercel-labs/skills)로 스킬을 직접 설치한다.

```bash
# 전체 스킬 설치
npx skills add hyoeun979704-web/AI_maketing_ofice

# 특정 스킬만 설치
npx skills add hyoeun979704-web/AI_maketing_ofice --skill page-cro copywriting

# 사용 가능한 스킬 목록
npx skills add hyoeun979704-web/AI_maketing_ofice --list
```

`.agents/skills/` 디렉터리에 자동 설치된다 (Claude Code 호환을 위해 `.claude/skills/`로 심볼릭 링크 생성).

### 옵션 2: Claude Code 플러그인

Claude Code 내장 플러그인 시스템 사용.

```bash
# 마켓플레이스 추가
/plugin marketplace add hyoeun979704-web/AI_maketing_ofice
...
```bash
git clone https://github.com/hyoeun979704-web/AI_maketing_ofice.git
...
```bash
git submodule add https://github.com/hyoeun979704-web/AI_maketing_ofice.git .agents/marketingskills
```

그 다음 `.agents/marketingskills/skills/`에서 스킬을 참조한다.
